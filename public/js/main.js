$(function () {
    ...
    $.get('/get_chatters', function (response) {
        $('.chat-info').text("Զրուցարանում այս պահին կա " + response.length + " մարդ ");
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 'OK') { //username doesn't already exists
                    ...
                    alert("Կներեք բայց ձեր անունով արդեն գրանցվել են ,ընտրեք մեկ այլ անուն");
                    $('#username').val('').focus();
                }
            }
        });
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 'OK') {
                    ...
                    $('.join-chat').show();
                    $('#username').val('');
                    alert('Դուք հաջողությամբ դուրս եկաք չատից');
                }
            }
        });
    });
    ...
    socket.on('count_chatters', function (data) {
        ...
        $('.chat-info').text("Զրուցարանում այս պահին կա " + chatter_count + " մարդ");
    });
});
